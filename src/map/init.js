...
import geometrySymbol from "./modules/Labels.js";

const options = {
  url:"http://localhost:8001/arcgis_js_v332_api/arcgis_js_api/library/3.32/3.32/init.js"
};
function ArcGIS() {
  this.map = null; // 地图
...
ArcGIS.prototype.init = function init($el) {
  // 加载地图必备样式文件
  loadCss("http://localhost:8001/arcgis_js_v332_api/arcgis_js_api/library/3.32/3.32/esri/css/esri.css");
  loadCss("http://localhost:8001/arcgis_js_v332_api/arcgis_js_api/library/3.32/3.32/dijit/themes/claro/claro.css");
  loadModules([
      "esri/map",
      // "tdlib/SDTDTLayer",
      // "tdlib/SDRasterLayer",
    ...
    .then(
      ([
        ...
        this.geometrySymbol = null; // 图形样式集合

        this.baseMap = {
          vectorMap: new ArcGISDynamicMapServiceLayer("http://113.140.66.226:33225/arcgis/rest/services/TDTSxBaseMap_XX/MapServer"), //矢量
          rasterMap: new ArcGISDynamicMapServiceLayer("http://113.140.66.226:33225/arcgis/rest/services/TDTSxImgMap/MapServer"), //影像
        ...
        // 设置初始化地图位置
        const startExtent = new Extent(104.79522416831787,31.43766039138211,112.00713075579517,39.90697432336191,new SpatialReference({ wkid: 4326 }));
        ...
        //   "http://113.140.66.226:33225/arcgis/rest/services/TDTSxBaseMap_XX/MapServer",
        //   {
        //     id: "bianjie", // 设置 id
        ...
        function mapReady() {
          ...
          // // 配置缓冲区参数
          // EsriConfig.defaults.geometryService = new GeometryService("http://113.140.66.226:33225/arcgis/rest/services/Utilities/Geometry");
        ...
        let showBuffer = (bufferedGeometries) => {
          ...
          bufferedGeometries.forEach(function(geometry) {
            ...
            var BufferTask = QueryTask("http://113.140.66.226:33225/arcgis/rest/services/TDTSxBaseMap_XX/MapServer/0");
            var Bufferquery = new Query();
            Bufferquery.returnGeometry = true;
            // 设置坐标系
            graphicBuffer.spatialReference = new SpatialReference(4326);
